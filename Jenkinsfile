pipeline {
  ...
  environment {
    BOT_NAME = 'coding_agent_proxy_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-coding_agent_proxy"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/coding_agent_proxy_bot/bin/coding_agent_proxy_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyCodingAgentProxy.Release.create()'"
          $RELEASE_BIN eval 'BotArmyCodingAgentProxy.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyCodingAgentProxy.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyCodingAgentProxy.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_coding_agent_proxy/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_coding_agent_proxy/releases/start_erl.data)
        fi
