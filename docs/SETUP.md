# Getting Started with bot_army_coding_agent_proxy

This guide walks you through setting up Coding Agent Proxy Bot for local development.
...
DATABASE_NAME=bot_army_coding_agent_proxy_dev
```
...
Coding Agent Proxy Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_coding_agent_proxy/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_coding_agent_proxy/skills/example.ex lib/bot_army_coding_agent_proxy/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyCodingAgentProxy.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_coding_agent_proxy.command.classify"]
...
```elixir
# In lib/bot_army_coding_agent_proxy/application.ex or similar

defmodule BotArmyCodingAgentProxy.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyCodingAgentProxy.Skills.Classify,
      BotArmyCodingAgentProxy.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_coding_agent_proxy/skills/my_skill.ex
...
```elixir
releases: [
  coding_agent_proxy_bot: [
    applications: [bot_army_coding_agent_proxy: :permanent]
  ]
]
```

This creates a release named `coding_agent_proxy_bot` that is deployed to `/opt/ergon/releases/coding_agent_proxy_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/coding_agent_proxy_bot/`
